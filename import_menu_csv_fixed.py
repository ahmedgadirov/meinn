"""
Fixed CSV Import System for Meinn Restaurant Menu AI Assistant.
Imports menu items from CSV with correct column structure and multilingual translations.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('menu_csv_import_fixed.log')
    ]
)
logger = logging.getLogger("meinn_ai.csv_import_fixed")
        ...
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            for row_num, row in enumerate(reader, 1):
                try:
                    ...
                    csv_category = row.get('Category Name', '').strip()
                    if not csv_category:
                        csv_category = row.get('Category EN', '').strip()
                    ...
                    available_str = row.get('Available', 'TRUE').strip().upper()
                    available = available_str in ['TRUE', '1', 'YES', 'Y']
                    
                    popular_str = row.get('Popular', 'FALSE').strip().upper()
...
def verify_import():
    ...
    try:
        ...
        if size_samples:
            logger.info("\nSample items with custom sizes:")
...
if __name__ == "__main__":
    logger.info("Starting fixed CSV menu import...")
