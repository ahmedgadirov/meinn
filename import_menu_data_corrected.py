#!/usr/bin/env python3
"""
Corrected Menu Data Import Script for Meinn Restaurant
Fixed column mapping and counts to match database schema
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('menu_import_corrected.log')
    ]
)
...
def import_menu_items(conn, csv_file_path):
    ...
    logger.info("Importing menu items with correct column mapping...")
    cursor = conn.cursor()
    ...
    try:
        ...
        for (base_name, category_id), items in groups.items():
            try:
                ...
                # Insert menu item with EXACTLY 30 values for 30 columns
                cursor.execute('''
                ...
                ''', (
                    ...
                    price_for_main,                                      # 5: price
                    image_url,                                           # 6: image_url
                    available,                                           # 7: available
                    popular,                                             # 8: popular
                    prep_time,                                           # 9: preparation_time
                    created_at,                                          # 10: created_at
                    updated_at,                                          # 11: updated_at
                    name_translations['name_az'],                        # 12: name_az
                    name_translations['name_en'],                        # 13: name_en
                    name_translations['name_ru'],                        # 14: name_ru
                    name_translations['name_tr'],                        # 15: name_tr
                    name_translations['name_ar'],                        # 16: name_ar
                    name_translations['name_hi'],                        # 17: name_hi
                    name_translations['name_fr'],                        # 18: name_fr
                    name_translations['name_it'],                        # 19: name_it
                    description_translations['description_az'],          # 20: description_az
                    description_translations['description_en'],          # 21: description_en
                    description_translations['description_ru'],          # 22: description_ru
                    description_translations['description_tr'],          # 23: description_tr
                    description_translations['description_ar'],          # 24: description_ar
                    description_translations['description_hi'],          # 25: description_hi
                    description_translations['description_fr'],          # 26: description_fr
                    description_translations['description_it'],          # 27: description_it
                    json.dumps(size_options, ensure_ascii=False),        # 28: size_options
                    json.dumps(size_prices, ensure_ascii=False),         # 29: size_prices
                    default_size                                         # 30: default_size
                ))
                item_count += 1
...
def main():
    ...
    logger.info("Starting corrected menu import from CSV...")
